...
def compute_3d_displacement_complete(vol1, vol2, bbox):
    ...
    for z_idx in range(cell_vol1.shape[0]):
        ...
        displacement_3d_cell[z_idx, :, :, 0] = flow[:, :, 0]  
        displacement_3d_cell[z_idx, :, :, 1] = flow[:, :, 1]  
...
def calculate_3d_stress_cell_region(strain_3d):
    """Calculate the 3D stress field (cell area) based on strain analysis"""
...
def calculate_traction_stress_cell(stress_3d, normal_vectors=None):
    """Calculate the traction stress τ = σ · n (in the cell region)"""
...
def analyze_stress_time_series(data, center_ratio=0.4, reference_time=0):
    ...
    # Store the maximum stress at each time point
    time_max_stress = np.zeros(n_time)
    stress_time_series = []
    
    # Extract the central areas at all time points
    center_volumes = []
    center_bboxes = []
    ...
    for t in range(n_time):
        if t == reference_time:
            ...
            # Find the maximum stress value of all the Z layers
        ...
        time_max_stress[t] = max_stress
       
    return time_max_stress, stress_time_series, center_bboxes[0]

def visualize_time_series_stress(time_max_stress, stress_time_series):
    """Visualization of the stress analysis results of time series"""
    
    n_time = len(time_max_stress)
    
    # Create a time series chart
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    
    # 1. The maximum stress varies with time.
    time_points = np.arange(n_time)
    axes[0, 0].plot(time_points, time_max_stress, 'bo-', linewidth=2, markersize=6)
    axes[0, 0].set_xlabel('time point')
    axes[0, 0].set_ylabel('Maximum stress (Pa)')
    axes[0, 0].set_title('The maximum stress varies with time.')
    axes[0, 0].grid(True, alpha=0.3)
    
    # Mark the maximum value
    max_idx = np.argmax(time_max_stress)
    axes[0, 0].plot(max_idx, time_max_stress[max_idx], 'ro', markersize=10, 
                   label=f'maximum: {time_max_stress[max_idx]:.2e} Pa')
    axes[0, 0].legend()
    
    # 2. Stress rate of change
    if n_time > 1:
        stress_growth = np.diff(time_max_stress)
    ...
    # 3. cumulative stress
    cumulative_stress = np.cumsum(time_max_stress)
    ...
    if any(stress_time_series):
        valid_stresses = [ts['max_stress'] for ts in stress_time_series if ts is not None]
        axes[1, 1].hist(valid_stresses, bins=min(10, len(valid_stresses)), alpha=0.7, color='orange')
        axes[1, 1].set_xlabel('Maximum stress (Pa)')
    ...
    for t in range(n_time):
        if t == 0:
            ...
            change = time_max_stress[t] - time_max_stress[t-1]
        print(f"{t}\t{time_max_stress[t]:.6e}\t{change:+.6e}")
    
    return fig

def create_stress_heatmap_time_series(stress_time_series, center_bbox, n_cols=4):
    """Create a time series stress heatmap"""
    
    n_time = len(stress_time_series)
    valid_times = [t for t in range(n_time) if stress_time_series[t] is not None]
    
    if not valid_times:
        print("There are no valid stress data available to show.")
        return
    
    n_rows = (len(valid_times) + n_cols - 1) // n_cols
    
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(4*n_cols, 4*n_rows))
    if n_rows == 1:
        axes = axes.reshape(1, -1)
    
    x, y, z, w, h, d = center_bbox
    
    for idx, t in enumerate(valid_times):
        row = idx // n_cols
        col = idx % n_cols
        
        stress_data = stress_time_series[t]
        traction_magnitude = stress_data['traction_magnitude']
        
        # Calculate the average stress of all Z layers
        avg_stress_2d = np.mean(traction_magnitude, axis=0)
        
        im = axes[row, col].imshow(avg_stress_2d, cmap='hot', aspect='equal')
        axes[row, col].set_title(f'Time point {t}\nMaximum stress: {stress_data["max_stress"]:.2e} Pa')
    ...
    for idx in range(len(valid_times), n_rows * n_cols):
        row = idx // n_cols
        col = idx % n_cols
        axes[row, col].axis('off')
    
    plt.suptitle('Time series stress distribution heatmap (average of all Z layers)', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    try:
        ...
        time_max_stress, stress_time_series, center_bbox = analyze_stress_time_series(
            data, center_ratio=0.4, reference_time=0)
       
        fig_time_series = visualize_time_series_stress(time_max_stress, stress_time_series)
        fig_heatmaps = create_stress_heatmap_time_series(stress_time_series, center_bbox)
